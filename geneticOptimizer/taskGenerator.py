...
class TaskGenerator:
    def __init__(self, dataPath, start_index, end_index, floors, groupBy, output_path) -> None:
        ...
        self.taskTypes = ["live", "forward", "backward"]
    ...
    def evaluateAILoss(self, chr, taskType): ## Forecasting error difference
        ...
        for zone, task in taskList.items():
            ...
            if taskType == "forward":
                ...
                lossPerZone = self.megaMemory[f"{taskType}Loss"][zone].evaluate(approximatedSet=task["approximatedSet"], supportSet=task["supportSet"], mode="live")
